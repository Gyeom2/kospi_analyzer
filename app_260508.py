...
st.markdown("""
    <style>
    /* 테이블 내의 모든 텍스트 크기를 조절하고 줄바꿈을 방지합니다 */
...
rotation_data = collector.get_sector_rotation(start_date, end_date)

if analysis_data:
    if rotation_data:
        st.subheader(f"3. 투자 주체별 상세 분석 ({start_date} ~ {end_date})")

        # 탭을 제거하고 외국인/기관 순서대로 세로 배치
        for investor_type in ["외국인", "기관"]:
            st.divider()
            ...
            delta_data = collector.get_intensity_delta(start_date, end_date)
        ...
        st.write("로테이션 데이터에 문제가 있습니다.")
        st.write(f"rotation_data: {rotation_data}")

    st.write(f"### 3-8. 🏆 수급 연속성 점수판 (연속 순매수 TOP10)")
    ...
    st.write(f"### 3-9. 📉 수급 리스크 점수판 (연속 순매도 TOP 10)")
    st.markdown("""
    > **🚨 리스크 관리 가이드**
...
if futures_data:
    ...
    # ---------------------------------------------------------
    # 4. 프로그램 매매 동향 섹션 추가 (기존 구조 하단에 자연스럽게 배치)
    # ---------------------------------------------------------
    if program_data:
...
st.divider()
st.subheader("5. 🎭 코스피200 옵션 및 포지션 분석")
...
if max_pain_data:
    current_futures_price = futures_data['price_trend'].iloc[-1]['종가'] if futures_data else 0
    ...
    with st.container():
        ...
        if not near_pain_df.empty:
            mp_near = near_pain_df.loc[near_pain_df['고통지수'].idxmin(), '행사가']
...
st.divider()
st.subheader("6. 🏛️ 국고채 금리 및 장단기 금리차")
...
# app.py 하단부
# if st.button("📥 PDF 분석 리포트 생성"):
#     with st.spinner("리포트를 생성 중입니다..."):
#         # 1. 데이터 준비 (이미 수집된 데이터 사용)
#         report_data = {
#             "max_pain": max_pain_data['max_pain'],
#             "calls": calls_near.head(10),
#             "puts": puts_near.head(10)
#         }
...
#                 label="📄 리포트 다운로드",
#                 data=f,
#                 file_name=f"Market_Report_{target_date}.pdf",
